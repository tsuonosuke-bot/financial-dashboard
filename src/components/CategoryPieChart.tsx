...
const COLORS = ['#4f46e5', '#06b6d4', '#f59e0b', '#ef4444', '#10b981', '#8b5cf6', '#ec4899', '#84cc16']
...
export function CategoryPieChart({ expenses, selectedMonth }: Props) {
  const data = useMemo(() => {
    ...
    const sorted = Array.from(totals.entries())
      .map(([category, total]) => ({ category: categoryLabel(category), total }))
      .sort((a, b) => b.total - a.total)
    const primary = sorted.slice(0, 7)
    const other = sorted.slice(7).reduce((sum, item) => sum + item.total, 0)
    return other > 0 ? [...primary, { category: 'その他', total: other }] : primary
  ...
  return (
    <section className="panel">
      ...
      {data.length === 0 ? (
        <div className="grid h-[300px] place-items-center text-sm text-slate-500">この月の支出データはありません</div>
      ) : (
