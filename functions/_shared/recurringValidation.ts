...
export async function readRecurringCreate(request: Request): Promise<Record<string, unknown> | Response> {
  ...
  if (!Number.isSafeInteger(value.source_expense_id) || Number(value.source_expense_id) <= 0) return jsonResponse({ error: '元明細が正しくありません。' }, 400)
  ...
  return {
    p_source_expense_id: value.source_expense_id,
